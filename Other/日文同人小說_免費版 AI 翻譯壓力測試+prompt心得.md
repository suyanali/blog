...
**缺點**：若你是先給prompt，再複製貼上要翻的文字，它第一句回覆一定會抽風，直接按停止鍵無視它，把要翻的內容貼上。建議傳TXT給它翻，文章過長會分段回覆。速度偏慢。遇到擋的內容，調整 prompt試試，不行就換google ai studio。

3. **ChatGPT（免費帳號）**   
**優點**：可以翻 R18（但尺度政策會擋），速度普通。翻譯有空行，有限的TXT上傳。


**缺點**：貼太長會出現「摘要模式」或斷章翻譯。字數輸入輸出有上限，另外，聊天次數也有上限，到上限會整個chatgpt停用一陣子(我第一次知道有對話上限)。需多次強調是要「全文翻譯」而不是「摘要」。使用破折號較多。
